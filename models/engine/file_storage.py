...
class FileStorage:
    ...
    def save(self):
        ...
        The method iterates through the __objects dictionary and converts each
    ...
    back into objects using the class information (__class__) and attributes
    ...
    If an object is provided as an argument, the method retrieves the key for
    the object in the format "{class_name}.{object_id}"
    and deletes the corresponding entry from the __objects dictionary.
    ...
    def close(self):
        """
    Closes the current storage session by reloading the objects from the JSON
    file.

    The method calls the reload() method to reload the objects from the JSON
    file and update the __objects dictionary.
    ...
    def get(self, objId):
        """
    Retrieves an object from the __objects dictionary based on the provided
    objId.

    The method iterates over the keys in the __objects dictionary and extracts
      the object ID from each key.
    If the extracted object ID matches the provided objId, the corresponding
    object is returned.
    ...
    Returns:
        The object corresponding to provided objId if found, None otherwise.
    """
        for key in self.all():
            result = re.sub(r'^.*?\.', '', key)
            if objId == result:
